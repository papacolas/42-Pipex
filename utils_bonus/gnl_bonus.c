...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/08 14:59:50 by ncrivell          #+#    #+#             */
/*   Updated: 2025/03/14 16:40:44 by ncrivell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_explore_text(char *principal_buff, int fd)
{
	...
	while (!ft_strchr(buff_reader, '\n') && bytes != 0)
	{
		...
		if (bytes < 0)
			return (freebuff(&principal_buff));
		buff_reader[bytes] = '\0';
		buff_temp = ft_strjoin_gnl(principal_buff, buff_reader);
		if (!buff_temp)
			return (freebuff(&principal_buff));
		free(principal_buff);
